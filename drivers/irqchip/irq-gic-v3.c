...
#ifdef CONFIG_PM
	unsigned int legacy_wakeup_irqs[32];
	unsigned int legacy_enabled_irqs[32];
#endif
};
...
int gic_v3_qcom_set_wake(struct irq_data *d, unsigned int on)
{
	...
	if (on)
		gic_data->legacy_wakeup_irqs[reg_offset] |=  1 << bit_offset;
	else
		gic_data->legacy_wakeup_irqs[reg_offset] &=  ~(1 << bit_offset);
...
void gic_v3_qcom_resume(void)
{
	...
	for (i = 0; i * 32 < gic_data.irq_nr; i++) {
		...
		writel_relaxed(gic_data.legacy_enabled_irqs[i],
			base + GICD_ISENABLER + i * 4);
	}
}
#endif
